...
response.appendHeader('Content-Type', 'application/json');
    ...
    try {
        ...
        console.log(`[archiveGroup] Archiving conversation SID: ${conversationSid}`);

        await client.conversations.v1.conversations(conversationSid).update({ state: 'closed' });

        response.setStatusCode(200);
        response.setBody({ success: true, message: 'Group archived (state set to closed).' });
        return callback(null, response);
    } catch (error) {
        console.error('[archiveGroup] Error:', error);
        response.setStatusCode(500);
        response.setBody({ success: false, message: 'Failed to archive group.', error: error.message });
        return callback(null, response);
    }
};
