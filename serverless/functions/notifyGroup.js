...
response.appendHeader('Content-Type', 'application/json');
    ...
    // --- Input Validation ---
    const { conversationSid, message } = event;
    if (!conversationSid || !message) {
        response.setStatusCode(400);
        response.setBody({ success: false, message: 'Missing required parameters: conversationSid and message are required.' });
        return callback(null, response);
    }

    try {
        const client = context.getTwilioClient();
        console.log(`[notifyGroup] Sending message to conversation SID: ${conversationSid}`);
        ...
        response.setBody({ success: true, message: 'Notification message sent to group.' });
        return callback(null, response);
    } catch (error) {
        console.error('[notifyGroup] Error:', error);
        response.setStatusCode(500);
        response.setBody({ success: false, message: 'Failed to send notification message.', error: error.message });
        return callback(null, response);
    }
};
